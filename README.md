...
Welcome to the **Woody Visual** Database Visualization Platform! This platform is dedicated to the visualization of plant single-cell UMAP (Uniform Manifold Approximation and Projection) and comprehensive gene expression data.
...
* **Introduction**: Learn about the research background, methodology, and primary purpose of this database.
...
This tab provides a high-level overview of the cell composition proportions across different plant species.
...
Access and download the raw data and associated databases used in our research through this portal.

* **Data Filters**: Precisely locate the dataset you need by configuring various parameters, including: 
  `Samples`, `SRA`, `GEO`, `Species`, `Tissue`, `Source`, `Genotype`, `Treatment`, `DataType`, and `Technology`.
...
1. **Select Species**: Choose your target species from the dropdown (e.g., `Egr`, `Lch`, `Ptr`, `Pal`, `Tar`).
2. **Cell Grouping**: Define the cell observation range by choosing either **Select All** or **Subgroup**.
3. **Select CellTypes**: *(Enabled only if 'Subgroup' was selected)* Choose specific CellTypes. Unselected clusters will be dimmed in the final visualization.

### 📍 Card 2: Expression Analysis
*Use this section to visualize UMI across cell populations.*
1. **Target Type**: Choose the identifier type (`Gene ID` or `Ortholog ID`).
...
### 📍 Card 3: Cellular Fingerprint
*Use this section to examine the entire gene profile of a specific single cell.*
...
* **UMAP Representation**: The left graph displays the complete UMAP and Cell Types for the species; the right graph displays the targeted Expression Plot highlighting the expression intensity of your selected genes.
* **Violin & Box Plots**: Contains four comprehensive distribution graphs. The top graph shows the normalized Expression Violin Plot. The bottom three sequentially display cell lineage box plots for *Ray*, *Vessel*, and *Fiber*.
* **Data Table**: Presents a highly detailed data matrix corresponding to your expression analysis, detailing `Barcode`, `UMAP-1`, `UMAP-2`, `Expression (UMI)`, and `CellType`. You can export this data using the **Download CSV** button.
...
3. **Select CellTypes**: Choose specific CellTypes (e.g., `Vessel element` or `Fusiform intermediate precursor`).
...
4. **Select Ortholog Group**: Choose the target ortholog group for observation (single selection).
5. **Execute**: Click **Plot Graph** to generate the comparative visualization.

### 📊 Charts and Data Panel
Located at the bottom of the page, review your multi-species comparison:

* **UMAP Presentation**: The left graph displays the complete UMAP for the cross-species combination; the right graph displays the filtered Expression Plot.
